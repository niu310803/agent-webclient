...
切换开始时，`useConversationActions` 先通过 AppContext 中的 viewport handle 同步保存来源 Chat 阅读位置，再创建事务。请求期间保留来源 timeline，不再用 `streaming=true` 模拟历史加载，也不提前清空事件；`ConversationStage` 对不同稳定 `chatId` 的显式历史切换始终覆盖骨架层，因此 Router 同步提交后的首个可绘制帧不会闪现旧 Chat，目标有 active run 也不例外。骨架从挂载起完整保持至少 160ms；数据在此之前 ready 时等待满 160ms 后开始 80ms 渐隐，数据更慢时在 ready 后立即开始 80ms 渐隐；`prefers-reduced-motion` 下保持 160ms 后直接移除。新目标会取消旧目标的 hold/fade timer。加载错误立即改为可重试错误层，不受最短显示时长限制。new Chat 首次创建、`newChat → chatId` canonical promotion 与同 Chat 的 `same-chat-reload` 后台恢复不触发历史骨架，并始终保留实时 Timeline。判断 live promotion 只认可 `observationSource !== "attach"` 的原 query session；若 route-driven 历史事务已与该 promotion 竞态创建，时间线立即清除该事务，使其迟到响应失效且不继续锁定 Composer。历史响应返回带非空 `runId` 的 `activeRun` 时，Chat ID、reset、replay 投影和 `currentChatActiveRun` 仍在同一个 `flushSync` 批次内应用，attach、增量事件接收和阅读位置恢复可在骨架后继续；显式历史切换只在事务 ready 且最短展示满足后揭示 Timeline，运行恰好完成也不会重新打开骨架。失败时来源数据继续保留在错误层后方，重试创建新事务，不提交空目标会话。
